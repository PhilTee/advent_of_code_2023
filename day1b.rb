...
def find_words(string)
  locations={}
  WORDS.each_with_index do |word, number|
    index = string.index(word)
    if index
      locations[index] = [word, (number+1).to_s]
    end
  end

  return string if locations.empty?

  first_word = locations[locations.keys.min]
  last_word = locations[locations.keys.max]

  string.gsub(first_word[0], first_word[1])
        .gsub(last_word[0], last_word[1])
end

def extract_first_and_last_digits(string)
  digits = string.chars.select { |c| NUMBERS.include?(c) }
...
digits = lines.map do |line|
  puts line
  digitised_line = find_words(line)
  puts digitised_line
  puts "\n"

  extract_first_and_last_digits(digitised_line)
end
