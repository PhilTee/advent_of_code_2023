...
@data = {}
@game_data = """Game 1: 10 green, 5 blue; 1 red, 9 green, 10 blue; 5 blue, 6 green, 2 red; 7 green, 9 blue, 1 red; 2 red, 10 blue, 10 green; 7 blue, 1 red
...
def ingest_data
  @game_data.lines.each do |line|

    game, data = line.chomp.split(':')
    game_id = game.split[1].to_i
    iterations = data.split(';')
    
#    puts "Game #{game_id} has #{iterations.count} iterations"

    @data[game_id] = []

    data = iterations.map do |iteration|
      colours = iteration.split(',')
      colour_hash = Hash.new(0)
      colours.each do |colour|
        count, hue = colour.strip.split
        colour_hash[hue] = count.to_i
      end
      colour_hash
    end
    @data[game_id] = data 
...
game_validator = @data.collect do |game_id, iterations|
  valid = true
  iterations.each do |iteration|
#    puts iteration.inspect
#    puts "R: #{iteration['red']} > #{MAX_CUBES['red']}"
#    puts "B: #{iteration['blue']} > #{MAX_CUBES['blue']}"
#    puts "G: #{iteration['green']} > #{MAX_CUBES['green']}"

    if (iteration['red'] > MAX_CUBES['red']) || (iteration['blue'] > MAX_CUBES['blue']) || (iteration['green'] > MAX_CUBES['green'])
      valid = false
    end
  end
  game_id if valid
...
puts game_validator.inspect
puts game_validator.sum
